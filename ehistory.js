...
EHistory.prototype.doSearch = function(query, options) {//@FIXME make one argument
	...
	_.extend(this.settings, {text: this.strippedSearchText_});
	if(options)
		_.extend(this.settings, options);

	var self = this;
	var searchSettings = {};
	_.extend(searchSettings, this.settings);
...
EHistory.loadInLocalStorage = function(){
	...
	var storeVisit = function(visits, hitem){
		for (var i=0; i < visits.length; i++){
			_.extend(visits[i], hitem);
			localStorage[visits[i].visitId] = JSON.stringify(visits[i]); 
		}
		
	};
	chrome.history.search({text:'', startTime:0, endTime: (new Date).getTime(), maxResults:99999999}, getVisits);
}
...
//console.log(results);
	...
	for (var i = 0; i < results.length; i++){
		...
		chrome.history.getVisits({url : results[i].url}, function(visits){self.fixAndSendBack_(visits);});
	}
}
...
EHistory.prototype.filterOutResults_ = function(results){
	...
	for (var i=0; i < results.length; i++)
	{
		...
		{
			_.extend(results[i], this.settings);
